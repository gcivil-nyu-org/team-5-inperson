...
class water_List(generics.ListAPIView):
    ...
    def get_queryset(self):        
        ...
        narrowed_list = [e for e in water_all if bottom_left_lng <= e.water_longitude <= top_right_lng and bottom_left_lat <= e.water_latitude <= top_right_lat]
        print("\nNumber of water resources : "+str(len(narrowed_list))+"\n")
        

        # Build your filtered queryset
        narrowed_queryset = water_all.filter(water_longitude__in=[e.water_longitude for e in narrowed_list],water_latitude__in=[e.water_latitude for e in narrowed_list])
...
class wifi_List(generics.ListAPIView):
    def get_queryset(self):        
        ...
        narrowed_list = [e for e in wifi_all if bottom_left_lng <= e.wifi_longitude <= top_right_lng and bottom_left_lat <= e.wifi_latitude <= top_right_lat]
        print("\nNumber of wifi resources : "+str(len(narrowed_list))+"\n")
        

        # Build your filtered queryset
        narrowed_queryset = wifi_all.filter(wifi_longitude__in=[e.wifi_longitude for e in narrowed_list],wifi_latitude__in=[e.wifi_latitude for e in narrowed_list])
        # Set the cache on the queryset
        #narrowed_queryset._result_cache = narrowed_list
...
class parking_List(generics.ListAPIView):
    def get_queryset(self):        
        ...
        narrowed_list = [e for e in parking_all if bottom_left_lng <= e.parking_longitude <= top_right_lng and bottom_left_lat <= e.parking_latitude <= top_right_lat]
        print("\nNumber of parking resources : "+str(len(narrowed_list))+"\n")
        

        # Build your filtered queryset
        narrowed_queryset = parking_all.filter(parking_longitude__in=[e.parking_longitude for e in narrowed_list],parking_latitude__in=[e.parking_latitude for e in narrowed_list])
        # Set the cache on the queryset
        #narrowed_queryset._result_cache = narrowed_list
...
class bench_List(generics.ListAPIView):
    def get_queryset(self):        
        ...
        narrowed_list = [e for e in bench_all if bottom_left_lng <= e.bench_longitude <= top_right_lng and bottom_left_lat <= e.bench_latitude <= top_right_lat]
        print("\nNumber of bench resources : "+str(len(narrowed_list))+"\n")
        

        # Build your filtered queryset
        narrowed_queryset = bench_all.filter(bench_longitude__in=[e.bench_longitude for e in narrowed_list],bench_latitude__in=[e.bench_latitude for e in narrowed_list])
        # Set the cache on the queryset
        #narrowed_queryset._result_cache = narrowed_list
...
class toilet_List(generics.ListAPIView):
    def get_queryset(self):        
        ...
        narrowed_list = [e for e in toilet_all if bottom_left_lng <= e.toilet_longitude <= top_right_lng and bottom_left_lat <= e.toilet_latitude <= top_right_lat]
        print("\nNumber of restroom resources : "+str(len(narrowed_list))+"\n")
        

        # Build your filtered queryset
        narrowed_queryset = toilet_all.filter(toilet_longitude__in=[e.toilet_longitude for e in narrowed_list],toilet_latitude__in=[e.toilet_latitude for e in narrowed_list])
        # Set the cache on the queryset
        #narrowed_queryset._result_cache = narrowed_list
